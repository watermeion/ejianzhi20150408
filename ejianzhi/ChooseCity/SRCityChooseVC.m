...
#import "SRCityChooseVC.h"

@interface SRCityChooseVC ()

@end

@implementation SRCityChooseVC
...
- (void)viewDidLoad {
    ...
    if ([mySettingData objectForKey:@"currentCity"]) {
        self.gpsCityLabel.text=[mySettingData objectForKey:@"currentCity"];
        _tableView.tableHeaderView=self.headView;
    }
    
    UITapGestureRecognizer *tapGesture=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickHeadView:)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.chooseDelegate finishChoose:[[_cities objectForKey:key] objectAtIndex:indexPath.row]];
    
    NSUserDefaults *mySettingData = [NSUserDefaults standardUserDefaults];
    [mySettingData setObject:[[_cities objectForKey:key] objectAtIndex:indexPath.row] forKey:@"currentCity"];
...
- (void)clickHeadView:(UITapGestureRecognizer *)gesture{
    ...
    NSUserDefaults *mySettingData = [NSUserDefaults standardUserDefaults];
    [mySettingData setObject:self.gpsCityLabel.text forKey:@"currentCity"];
    [mySettingData synchronize];

    [self.chooseDelegate finishChoose:self.gpsCityLabel.text];
    [self.navigationController popViewControllerAnimated:YES];
